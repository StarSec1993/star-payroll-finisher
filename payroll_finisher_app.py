                ...
                if st.session_state.get('payroll_processed', False):
                    ...
                    if 'php_warnings' in stats and len(stats['php_warnings']) > 0:
                        ...
                        with st.expander(f"📋 View {len(stats['php_warnings'])} Employees — PHP Included"):
                            ...
                            warning_text += "- Employee terminated before payroll period\n"
                            warning_text += "- Employee hired after payroll period started\n\n"
                            warning_text += "**Action needed:** Manually calculate and add PHP for eligible employees.\n\n"
                            warning_text += "**Affected Employees:**\n"
                            for warning in sorted(stats['php_warnings']):
                                parts = warning.split('|')
                                employee_name = parts[0]
                                stat_date = parts[1] if len(parts) > 1 else "Unknown"
                                warning_text += f"- {employee_name} (Stat: {stat_date})\n"
